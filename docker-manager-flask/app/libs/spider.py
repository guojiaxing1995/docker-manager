...
def deal_default(depend_data_dict,data,target_key=None,new_key=None,true_key=None):
    if type(data) == list:
        for i in data:
            deal_default(depend_data_dict,i,target_key,new_key)
    if type(data) == dict:
        for key,value in data.items():
            deal_default(depend_data_dict,value,target_key,new_key,key)
    if type(data) == str:
        if target_key:
            if target_key == true_key:
                if new_key:
                    depend_data_dict[new_key] = data
                else:
                    depend_data_dict[true_key] = data
        else:
            depend_data_dict[true_key] = data
    if type(data) == int:
        if target_key:
...
d = {}
print(deal_default(d,douban))
l = '(a,a1)(a,a1)(a,a1)(a,a1)(a,a1)'.split(')')
print(l[0][1:])
